...
import { Copy, ExternalLink, Send, Clock, CircleDot, Upload, ChevronDown, Share2, AlertTriangle, X } from "lucide-react";
    ...
    <>
      ...
      {exportWalletOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/60" onClick={() => setExportWalletOpen(false)} />
          <div className="relative bg-bg-surface rounded-2xl p-6 max-w-[440px] w-full mx-4 shadow-2xl border border-border-subtle">
            ...
            <div className="text-center mb-5">
              <h3 className="text-text-primary text-lg font-bold">Export wallet</h3>
              <p className="text-text-secondary text-sm mt-2">
                Copy either your private key or seed phrase to export your wallet. Learn more
              </p>
            </div>
            ...
            {/* Your wallet */}
            <div className="mb-4">
              <label className="text-text-secondary text-xs block mb-2">Your wallet</label>
              <div className="flex items-center justify-between bg-bg-input rounded-xl px-4 py-3 border border-border-subtle">
                <span className="text-text-primary text-sm font-mono">{MOCK_USER.shortAddress}</span>
                <button
                  onClick={handleCopy}
