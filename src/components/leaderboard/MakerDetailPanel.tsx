    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2">
          <h3 className="text-text-primary text-lg font-bold font-mono">
            {maker.address}
          </h3>
          <button
            onClick={handleCopy}
          ...
          <button
            onClick={() =>
              toast.info("Opening Kosmyklytics...", {
                description: "View detailed analytics for this maker.",
              })
            }
          ...
          <button
            onClick={() =>
              toast.info("Opening BaseScan...", {
                description: "View on-chain activity for this address.",
              })
            }
            className="text-text-secondary text-xs hover:text-accent-purple transition-colors flex items-center gap-1"
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-bg-input rounded-lg p-3">
          ...
          <p className="text-text-primary text-base font-bold mt-1 tabular-nums">
            {formatUSD(maker.filledVolume)}
          </p>
        </div>
        <div className="bg-bg-input rounded-lg p-3">
          <span className="text-text-secondary text-[10px] uppercase tracking-wider font-medium">
          ...
          <p className="text-accent-green text-base font-bold mt-1 tabular-nums">
            {formatUSD(maker.realizedProfit)}
          </p>
        </div>
        <div className="bg-bg-input rounded-lg p-3">
          <span className="text-text-secondary text-[10px] uppercase tracking-wider font-medium">
          ...
          <p className="text-text-primary text-base font-bold mt-1 tabular-nums">
            {formatUSD(maker.grossDeposited)}
          </p>
        </div>
        <div className="bg-bg-input rounded-lg p-3">
          <span className="text-text-secondary text-[10px] uppercase tracking-wider font-medium">
      ...
      <div>
        ...
        <p className="text-text-secondary text-xs mt-0.5">
          Rails this maker actively manages liquidity on
        </p>

        <div className="mt-4 space-y-3">
          {maker.platforms.map((ps) => {
            const platform = PAYMENT_METHODS.find(
              (p) => p.id === ps.platformId,
            );
              ...
              <div key={ps.platformId} className="flex items-center gap-3 select-none">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-text-tertiary text-xs">
                    {formatNumber(ps.fulfillments, 0)} fulfillments{" "}
                    {ps.manual > 0 ? `\u00B7 ${ps.manual} manual` : ""}
                  </p>
                </div>
                <div className="text-right shrink-0">
                  ...
                  <p className="text-accent-green text-xs tabular-nums">
                    {formatUSD(ps.profit)} profit
                  </p>
                </div>
              </div>
            );
